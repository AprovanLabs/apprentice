// Widget Generator - LLM-based widget generation with validation and compilation

import { readFile, writeFile, unlink, readdir } from 'node:fs/promises';
import { existsSync } from 'node:fs';
import { join, extname } from 'node:path';
import { smartComplete } from '../../ai/client.js';
import { getPatchworkConfig, ensureWidgetsDir } from '../config.js';
import { loadWidget, clearMetaCache } from '@aprovan/patchwork';
import type { WidgetMeta, WidgetRuntime } from '@aprovan/patchwork';
import { getPromptForRuntime, buildGenerationPrompt } from './prompts.js';
import { compileWidget, extractMetaFromSource } from './compiler.js';
import { validateWidget, type ValidationResult } from './validator.js';
...
export async function generateWidget(
  ...
  const { description, runtime, save = false } = options;
  ...
  let code: string;
  try {
    const result = await smartComplete(userPrompt, systemPrompt, {
      temperature: 0.3,
      maxTokens: 4000,
    });
...
export async function deleteWidget(name: string): Promise<boolean> {
  ...
  try {
    ...
    const config = getPatchworkConfig();
    const source = await readFile(widget.path, 'utf-8').catch(() => null);
    if (source && widget.runtime === 'browser') {
      const { hash } = await compileWidget(source);
      const cachePath = join(config.widgetsDir, '.cache', `${hash}.js`);
...
export async function exportWidget(
  name: string,
): Promise<{ name: string; code: string; meta: WidgetMeta } | null> {
  const result = await getWidget(name);
  if (!result) return null;

  const loadResult = await loadWidget(result.info.path);
  if (!loadResult.widget) return null;

  return {
    name: result.info.name,
    code: result.code,
    meta: loadResult.widget.meta,
  };
}
