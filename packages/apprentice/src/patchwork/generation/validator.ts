// Widget Security Validator - AST-based security scanning
...
function checkNode(node: ts.Node, ctx: ValidatorContext): void {
  ...
  if (ts.isImportDeclaration(node)) {
    ...
    if (ts.isStringLiteral(moduleSpec)) {
      ...
      if (pkg.startsWith('.') || pkg.startsWith('/') || pkg.startsWith('@/')) {
        return; // Relative/local imports OK
      }
  ...
  // Check new expressions (XMLHttpRequest, Worker, etc)
  ...
  // Check property access for dangerous globals
  ...
  // Check dangerous global references
